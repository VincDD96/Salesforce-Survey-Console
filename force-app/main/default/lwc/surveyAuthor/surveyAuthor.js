...
export default class SurveyAuthor extends LightningElement {
  ...
  @track showNewSurveyForm = false;
  @track newSurveyName = "";
  @track exportingResponses = false;
  ...
  handleNewSurveyToggle() {
    this.showNewSurveyForm = !this.showNewSurveyForm;
    this.newSurveyName = "";
  }
  ...
  async handleCreateSurvey() {
    const name = (this.newSurveyName || "").trim();
    if (!name) {
      this.toast("error", "Inserisci il nome del nuovo survey");
      return;
    }
    try {
      const created = await createRecord({
        apiName: SURVEY_API,
        fields: { Name: name, Status__c: "Draft" }
      });
      this.showNewSurveyForm = false;
      this.newSurveyName = "";
      this.selectedQuestionId = undefined;
      this.validation = undefined;
      this.pendingPositions.clear();
      this.surveyId = created.id;
      this.toast("success", 'Survey "' + name + '" creato (Draft)');
    } catch (e) {
  ...
  async handleAddQuestion() {
    ...
    await this.tryDml(
      () => createRecord({ apiName: QUESTION_API, fields }),
      "Domanda creata"
    );
    ...
    await this.tryDml(
      () =>
        updateRecord({
          fields: {
            Id: this.surveyId,
            Start_Question__c: this.selectedQuestionId
          }
        }),
  ...
  get exportDisabled() {
    return !this.surveyId || this.exportingResponses;
  }

  async handleExportResponses() {
    if (!this.surveyId) return;
    this.exportingResponses = true;
    try {
      // The CSV is generated and stored as a File server-side, then we just
      // navigate to its download URL — a client-side Blob download here
      // gets rejected by Lightning Web Security ("Unsupported MIME type").
      const url = await exportResponsesFile({ surveyId: this.surveyId });
  ...
  async handleSvgMouseUp(event) {
    ...
    const node =
      this.dragState && this.dragState.kind === "node" ? this.dragState : null;
  ...
  async handleDeleteQuestion() {
    ...
    const ok = window.confirm(
      "Eliminare questa domanda e tutte le sue opzioni?"
    );
    if (!ok) return;
  ...
  handleQuestionDefaultNextChange(event) {
    if (!this.selectedQuestionId) return;
    const raw = event.target.value || null;
    this.applyQuestionLocal(
      this.selectedQuestionId,
      "Default_Next_Question__c",
      raw
    );
  ...
  async handleAddOption() {
    ...
    const fields = {
      ...
      Order__c: (this.inspectorQuestion?.options?.length || 0) + 1,
      Allows_Free_Text__c: false
    };
    await this.tryDml(
      () => createRecord({ apiName: OPTION_API, fields }),
      "Opzione creata"
    );
    await this.handleRefresh();
  }
  ...
  computeLayout() {
    ...
    if (this.validation && this.validation.cycles) {
      for (const cycle of this.validation.cycles)
        for (const id of cycle) cyclesSet.add(id);
    }
    ...
    for (const q of this.survey.questions) {
      ...
      if (this.survey.startQuestionId && !reachable.has(q.id))
        cls.push("is-orphan");
    ...
    for (const node of nodes) {
      for (const row of node.rows) {
        if (!row.nextQuestionId || !positionsMap.has(row.nextQuestionId))
          continue;
  ...
  bfsPositions(questions, rowsByQuestion) {
    ...
    const depth = new Map();
    const seed =
      this.survey.startQuestionId && byId.has(this.survey.startQuestionId)
  ...
  titleGeometry(text) {
    ...
    const titleHeight =
      TITLE_TOP_PAD + lines.length * TITLE_LINE_HEIGHT + TITLE_BOTTOM_PAD;
