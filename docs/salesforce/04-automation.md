...
- **Enforcement alla pubblicazione**: l'esigenza è considerata **già coperta** dai meccanismi esistenti (bottone "Valida" nell'editor + eccezione runtime in `loadActiveSurveyByName` che impedisce comunque la compilazione di un survey Active senza start question). Nota: nel repo non esiste un blocco *automatico* al passaggio a `Active`; l'unico rafforzamento deciso è la validation rule su `Start_Question__c` censita in [07-roadmap.md](07-roadmap.md) — se in discussione si volesse estendere il blocco anche a cicli/orfani, va aggiunto lì.
